...
export class PayloadManager {
  ...
  /**
   * 根据级别生成并返回一个主要的 payload 对象
   * @param level 检测级别
   * @param purpose 用途：'reflected' | 'stored'
   */
  generatePayloadByLevel(level: 'LOW' | 'MEDIUM' | 'HIGH', purpose: 'reflected' | 'stored' = 'reflected'): XssPayload {
    const payloads = this.generatePayloadsByLevel(level, purpose);
    // 返回第一个生成的 payload 对象（通常是 basic 类型）
    return payloads[0];
  }
