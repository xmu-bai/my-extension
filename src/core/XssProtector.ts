...
export class XssProtector {
  ...
  filterDangerousHtml(html: string): string {
    ...
    Log.debug('XssProtector', 'HTML 内容已过滤潜在危险元素');
    return filtered;
  }
  ...
  startDomProtection() {
      ...
      mutations.forEach(mutation => {
        mutation.addedNodes.forEach(node => {
          ...
          if (element.tagName === 'SCRIPT' || element.tagName === 'IFRAME') {
            element.remove();
            Log.warn('XssProtector', `拦截并移除危险标签: ${element.tagName}`);
          }
          ...
          Array.from(element.attributes).forEach(attr => {
            if (attr.name.startsWith('on')) {
              ...
              Log.warn('XssProtector', `移除危险事件属性: ${attr.name}`);
            }
          });
        });
      });
    ...
    Log.info('XssProtector', 'XSS 防护已启用');
    return observer;
  }
}