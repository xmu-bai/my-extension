...
// 初始化白名单开关状态并绑定事件
function initWhitelistToggle() {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    const tab = tabs && tabs[0];
    if (!tab || !tab.url || tab.url.startsWith('chrome://') || tab.url.startsWith('chrome-extension://')) {
      // 在不可检测页面隐藏白名单开关
      const item = document.getElementById('whitelistSwitch').closest('.switch-item');
    ...
    chrome.storage.local.get(['config'], (result) => {
      const config = result.config || getDefaultConfig();
      const list = Array.isArray(config.whitelist) ? config.whitelist : [];
      const isIn = list.some(d => normalizeHost(d) === host || matchHostByRule(host, d));
      const el = document.getElementById('whitelistSwitch');
      const status = document.getElementById('whitelistSwitchStatus');
      el.checked = isIn;
      status.textContent = isIn ? 'ON' : 'OFF';
      status.style.color = isIn ? '#4CAF50' : '#666';

      el.addEventListener('change', (e) => {
        const enable = e.target.checked;
        chrome.storage.local.get(['config'], (res2) => {
          const cfg = res2.config || getDefaultConfig();
          const list = Array.isArray(cfg.whitelist) ? cfg.whitelist.slice() : [];
          const normalizedHost = host; // already normalized
          const hasExact = list.some(item => normalizeHost(item) === normalizedHost);
          if (enable) {
            if (!hasExact) list.push(normalizedHost);
          } else {
            // 仅移除精确项，不移除通配（*.example.com）
            for (let i = list.length - 1; i >= 0; i--) {
              const item = list[i];
              if (!item.includes('*') && normalizeHost(item) === normalizedHost) {
                list.splice(i, 1);
              }
            }
          }
          ...
          chrome.storage.local.set({ config: cfg }, () => {
            // 通知background更新（触发 DNR 重建）
            chrome.runtime.sendMessage({ action: 'update_config', config: cfg });
            // 更新状态文本
            status.textContent = enable ? 'ON' : 'OFF';
            status.style.color = enable ? '#4CAF50' : '#666';
          });
        });
      });
    });
...
// 本文件内轻量复制的白名单工具（与 background/whitelist 保持一致）
